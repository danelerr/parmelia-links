...
import { processAlchemyCustomWebhook } from "../services/alchemyCustomWebhook";
...
ingestRoutes.post("/alchemy/custom", async (c) => {
	const rawBody = await c.req.text();
	const result = await processAlchemyCustomWebhook(
		c.env,
		rawBody,
		c.req.header("X-Alchemy-Signature"),
	);
	if (result.status === "disabled") return c.notFound();
	if (result.status === "invalid_signature") {
		logWarn("alchemy_custom_webhook_rejected", {
			requestId: c.get("requestId"),
			reason: "invalid_signature",
		});
		return c.json({ error: "Invalid webhook signature" }, 401);
