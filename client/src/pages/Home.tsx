...
export default function Home({ user, previewModel }: { user: User; previewModel?: HomeReadModel }) {
	...
	const available = balances[selectedToken];
	...
	return (
		<main id="main-content" className="app-frame relative mx-auto flex min-h-dvh w-full max-w-[480px] flex-col px-5 pt-[calc(env(safe-area-inset-top)+1.5rem)] pb-[calc(env(safe-area-inset-bottom)+7.75rem)] animate-fade-up">
			...
			<section aria-labelledby="available-heading" className="meli-balance-card-app p-5">
				<div className="mb-3 flex items-center justify-between gap-3">
					...
					<div className="flex shrink-0 items-center gap-1">
						<TokenSelect value={selectedToken} options={activeNetwork.currencies} balances={balances} hideBalances={hideBalance} onChange={setSelectedToken} />
				...
				<p className="mt-2 text-[11px] text-text-faint">{selectedToken} · {selectedToken === "USDC" ? t("home.availableHint") : t("home.assetBalanceHint")}</p>
			...
			<div className="meli-quick-grid mt-6">
				...
				<HomeAction to="/send" label={t("home.send")} icon={<SendIcon />} />
				<HomeAction to="/swap" label={t("move.swapTitle")} icon={<SwapIcon />} />
			...
			<PrimaryNav />
			{detailsOpen ? <AccountDetailsSheet walletAddress={walletAddress} onClose={() => setDetailsOpen(false)} /> : null}
...
function AccountDetailsSheet({ walletAddress, onClose }: { walletAddress: string | null; onClose: () => void }) {
	const { t } = useTranslation();
	const dialogRef = useDialog<HTMLDivElement>(onClose);
	function copyAddress() { if (walletAddress) void navigator.clipboard.writeText(walletAddress).then(() => notifySuccess(t("settings.addressCopied"))); }
	return createPortal(<div className="dialog-backdrop fixed inset-0 z-50 flex items-end justify-center px-5" style={{ paddingBottom: "max(1.25rem, env(safe-area-inset-bottom))" }} onClick={onClose}><div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="account-details-title" tabIndex={-1} onClick={(event) => event.stopPropagation()} className="dialog-panel w-full max-w-sm p-5 animate-sheet-up"><div className="sheet-handle mb-5" aria-hidden="true" /><div className="mb-5 flex items-start justify-between"><div><p className="meli-kicker">{activeNetwork.isTestnet ? "Alpha" : t("home.account")}</p><h2 id="account-details-title" className="mt-2 font-display text-[24px]">{t("home.technicalDetails")}</h2></div><button type="button" onClick={onClose} aria-label={t("common.close")} className="meli-square-action h-11 w-11">×</button></div><dl className="border border-border text-[13px]"><div className="border-b border-border p-3"><dt className="font-mono text-[9px] uppercase tracking-[0.08em] text-text-faint">{t("signing.network")}</dt><dd className="mt-1 text-text">{activeNetwork.name} · {activeNetwork.chainId}</dd></div><div className="p-3"><dt className="font-mono text-[9px] uppercase tracking-[0.08em] text-text-faint">{t("settings.address")}</dt><dd className="mt-1 break-all font-mono text-[12px] text-text">{walletAddress || "—"}</dd></div></dl><div className="mt-5 grid grid-cols-2 gap-3"><button type="button" onClick={copyAddress} disabled={!walletAddress} className="btn btn-ghost">{t("common.copy")}</button>{walletAddress ? <a href={`${activeNetwork.explorerBaseUrl}/address/${walletAddress}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost">{t("settings.viewExplorer")}</a> : null}</div><p className="mt-4 text-[11px] leading-relaxed text-text-faint">{t("home.assistedRecovery")}</p></div></div>, document.body);
}

function HomeAction({ to, label, icon }: { to: string; label: string; icon: ReactNode }) {
