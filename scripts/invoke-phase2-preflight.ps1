...
if (-not (Test-Path -LiteralPath $protectedSecretPath)) {
	throw "The existing DPAPI-protected Payments secret file is required for semantic target verification"
}
...
try {
	...
	$arguments = @("preflight:phase2:remote")
	if ($Json) { $arguments += "--json" }
	& (Get-Command pnpm.cmd).Source @arguments
